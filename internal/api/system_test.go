...
func TestHandleSystemRedactsPodmanError(t *testing.T) {
	...
	srv := httptest.NewServer(New(st, &fakeDeployer{st: st}, fakePinger(leaky), "test-version", testSeal, testOpen, &fakeRoutesApplier{}, unusedLogSource, nil, nil, unusedStatsSource, unusedAllStatsProvider))
	t.Cleanup(srv.Close)
