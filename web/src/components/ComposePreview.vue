...
<template>
  <div class="flex flex-col gap-4">
    <div class="flex flex-wrap items-center justify-between gap-2">
      <div>
        <h3 class="text-sm font-medium text-content-primary">
          Project <span class="font-mono text-content-primary">{{ plan.project }}</span>
        </h3>
    ...
    >
      <template #description>
        <ul class="mt-1 flex flex-col gap-1">
          <li v-for="(w, i) in plan.warnings" :key="`top-${i}`" class="text-xs text-status-deploying">{{ w }}</li>
        </ul>
      </template>
    </UAlert>
    ...
    >
      <template #description>
        <p class="mt-1 text-xs text-status-deploying">
        ...
        <ul class="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5">
          <li v-for="slug in plan.orphans" :key="slug" class="font-mono text-xs text-status-deploying">{{ slug }}</li>
        </ul>
      </template>
    </UAlert>

    <div class="grid gap-3 sm:grid-cols-2">
      <div v-for="service in plan.services" :key="service.name" class="flex flex-col gap-2 rounded-lg border border-line p-3">
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0">
            <p class="truncate font-mono text-sm font-medium text-content-primary">{{ service.name }}</p>
            <p class="truncate font-mono text-xs text-content-muted" :title="service.slug">{{ service.slug }}</p>
          </div>
        ...
        <div class="flex flex-col gap-1 rounded-md border border-line/60 bg-surface-elevated/40 px-2.5 py-2 text-xs">
          <div v-if="service.image" class="flex min-w-0 items-center gap-1.5">
            <UIcon name="i-lucide-box" class="size-3.5 shrink-0 text-content-muted" />
            <span class="truncate font-mono text-accent" :title="service.image">{{ service.image }}</span>
          </div>
          <div v-else-if="service.build" class="flex min-w-0 items-center gap-1.5">
            <UIcon name="i-lucide-hammer" class="size-3.5 shrink-0 text-content-muted" />
            <span class="truncate font-mono text-accent" :title="service.build.context">
              build: {{ service.build.context }}
            </span>
            <span v-if="service.build.dockerfile" class="shrink-0 truncate font-mono text-content-muted">
          ...
          <div v-if="service.volumes?.length" class="flex flex-col gap-0.5">
            <div v-for="v in service.volumes" :key="v.name" class="flex min-w-0 items-center gap-1.5">
              <UIcon name="i-lucide-database" class="size-3.5 shrink-0 text-content-muted" />
              <span class="truncate font-mono text-content-secondary">{{ v.name }}</span>
              <span class="shrink-0 text-content-muted">&rarr;</span>
              <span class="truncate font-mono text-content-secondary" :title="v.path">{{ v.path }}</span>
            </div>
          </div>

          <div v-if="service.env_keys?.length" class="flex flex-wrap items-center gap-1 pt-0.5">
            <UIcon name="i-lucide-key-round" class="size-3.5 shrink-0 text-content-muted" />
        ...
        <ul v-if="service.warnings?.length" class="flex flex-col gap-1">
          <li v-for="(w, i) in service.warnings" :key="i" class="text-xs text-status-deploying">{{ w }}</li>
        </ul>
      </div>
    </div>
